...
type Props = { station: Station; now: number; index: number; selected: boolean; onSelect: () => void; onBrew: () => void; onServe: () => void; onTrash: () => void; onDropIngredient: (ingredient: Ingredient) => void }

const isIngredient = (value: string): value is Ingredient => ['bean', 'milk', 'sugar'].includes(value)

export function BrewStation({ station, now, index, selected, onSelect, onBrew, onServe, onTrash, onDropIngredient }: Props) {
  ...
  return <article className={`brew-station ${selected ? 'selected' : ''} ${ready ? 'ready' : ''}`} onClick={onSelect} onDragOver={(event) => event.preventDefault()} onDrop={dropIngredient}>
    ...
    {station.status === 'empty' ? <button className="brew-button" onClick={(event) => { event.stopPropagation(); onBrew() }}>BREW</button> : station.status === 'overflow' ? <button className="trash-button" onClick={(event) => { event.stopPropagation(); onTrash() }}>TOSS CUP</button> : <button className={`serve-button ${ready ? 'lit' : ''}`} onClick={(event) => { event.stopPropagation(); onServe() }}>{ready ? 'SERVE!' : 'POURING'}</button>}
  </article>
}
